...
- Run `git diff` to see what changed (staged and unstaged)
- Run `git log --oneline -5` to see the recent commit style
...
- Draft a commit message from the changes, matching the repo's existing commit style
...
- If the commit fails (e.g. a pre-commit hook), fix the issue and try again with a NEW commit

## Step 3: Push

- Check whether the current branch has an upstream remote
- If it doesn't, propose creating one with `git push -u origin <branch>`
- **ASK the user to confirm** before pushing
- Only after confirmation: push to the remote

## Step 4: Pull Request

- Check whether a PR already exists for this branch (`gh pr view`); if one does, show the URL and stop
- Analyze ALL commits on this branch against the base branch, not just the latest commit
...
Follow the **PR Review Discipline** rule. The critical point: a green, "clean," mergeable PR is **not** ready to merge while a reviewer is still running — Copilot or Codex can be mid-review with no comments posted yet. Wait for the round to *finish*, not just for checks to pass.
...
1. **Wait for the review round to finish.** `gh` has no watch for reviews, so poll: sleep ~2 min between checks, for up to ~10 min.
   ```
   ...
   - **Timeout path (deterministic):** if ~10 min passes and an expected reviewer still hasn't posted for the current commit, re-request that reviewer once. If it still doesn't engage after another short wait, stop polling and tell the user which reviewer is missing, then ask whether to wait longer or proceed without it. Never wait indefinitely, and never count a missing reviewer as approval when merging.
2. **Address each comment:** implement it, or decline with a short reply. For each comment you handle, **resolve the thread** (`gh api graphql` → `resolveReviewThread`). **Don't resolve** a thread that asks a question or for more info — reply and leave it open.
3. **A fix push is a new push to review.** Resolve the fixed threads, re-request in one comment, then go back to step 1:
   ```
   gh pr comment <n> --body "Addressed the feedback — @claude review, @codex review, @codex[agent] review, @copilot review"
   ```
4. **Merge only when** every reviewer has *finished* the latest round with no remaining comments, all threads are resolved, CI is green (`gh pr checks <n> --watch --fail-fast`), **and** the user explicitly confirms. Never merge while a review is in progress or on silence.
