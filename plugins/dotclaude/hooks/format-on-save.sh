...
# Prefer `uv run ruff` when this is a uv project — ruff is usually only
...
if [ "$FORMATTED" = false ] && [ -n "$RUFF" ]; then
  ...
  if [ "$HAS_RUFF_CONFIG" = true ]; then
    case "$EXTENSION" in
      py)
        # $RUFF is intentionally word-split ("uv run ruff" or "ruff").
        # shellcheck disable=SC2086
        (cd "$ROOT" && $RUFF format "$FILE_PATH" >/dev/null 2>&1)
        # shellcheck disable=SC2086
        (cd "$ROOT" && $RUFF check --fix "$FILE_PATH" >/dev/null 2>&1)
        FORMATTED=true
        ;;
    esac
  fi
